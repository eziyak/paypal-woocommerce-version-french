...
class WC_Gateway_PayPal_Credit_Card_Rest_AngellEYE extends WC_Payment_Gateway_CC {
    ...
    function __construct() {
        ...
        $this->method_description = __('Paiements directs par carte de crédit PayPal en utilisant lAPI REST. Cela vous permet daccepter les cartes de crédit directement sur le site sans avoir besoin de la totalité des Paiements Pro.', 'paypal-for-woocommerce');
        $this->supports = array(
            'subscriptions',
            'products',
            'refunds',
        ...
        if ($this->is_valid_for_use_paypal_credit_card_rest()) {
            ...
            <?php 
                if(version_compare(WC_VERSION,'2.6','<')) {
                    ...
                    echo '<div class="error angelleye-notice" style="display:none;"><div class="angelleye-notice-logo"><span></span></div><div class="angelleye-notice-message">' . __('PayPal pour WooCommerce nécessite PHP version 5.3.0 ou supérieure.','paypal-for-woocommerce') . '</div></div>';
                } else {
            ...
            ?><div class="error angelleye-notice" style="display:none;"><div class="angelleye-notice-logo"><span></span></div><div class="angelleye-notice-message"><strong><?php _e('Gateway Disabled', 'paypal-for-woocommerce'); ?></strong>: <?php _e('PayPal ne prend pas en charge la devise de votre magasin.', 'paypal-for-woocommerce'); ?></div></div> <?php
        }
    }
    ...
    public function save_payment_method_checkbox() {
        printf(
                ...
                esc_attr( $this->id ),
                apply_filters( 'cc_form_label_save_to_account', __( 'Enregistrer la méthode de paiement sur mon compte', 'woocommerce' ), $this->id)
        );
    }
        ...
        try {
            if (isset($_POST['wc-paypal_credit_card_rest-payment-token']) && 'new' !== $_POST['wc-paypal_credit_card_rest-payment-token']) {
                ...
                if ($token->get_user_id() !== get_current_user_id()) {
                    throw new Exception(__('Erreur lors du traitement de la commande. Veuillez réessayer.', 'paypal-for-woocommerce'));
                } else {
                    return true;
                }
            }
            ...
            if (empty($card->exp_month) || empty($card->exp_year)) {
                throw new Exception(__('La date dexpiration de la carte est invalide', 'woocommerce-gateway-paypal-pro'));
            }
            if (!ctype_digit($card->cvc)) {
                throw new Exception(__('Le code de sécurité de la carte est invalide (seuls les chiffres sont autorisés)', 'woocommerce-gateway-paypal-pro'));
            }
            ...
            if (empty($card->number) || !ctype_digit($card->number)) {
                throw new Exception(__('Le numéro de carte est invalide', 'woocommerce-gateway-paypal-pro'));
            }
            return true;
        } catch (Exception $e) {
